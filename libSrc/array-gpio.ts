 ...
 ********************************************/
  ...
  setInput() {
    ...
    if (
      ...
      (arguments[0].pin === undefined || arguments[0].pin[0] == undefined)
    ) {
    ...
    if (
      ...
      for (var x = 0; x < arguments[0].pin.length; x++) {
        ...
        if (
          Number.isInteger(arguments[0].pin[x]) &&
          arguments[0].pin[x] > 0 &&
          arguments[0].pin[x] < 41
        ) {
          try {
    ...
    if (Number.isInteger(arguments[0]) && arguments.length >= 1) {
      for (var x = 0; x < arguments.length; x++) {
        ...
        // invalid pin number
          ...
          (Number.isInteger(arguments[x]) && arguments[x] <= 0) ||
          arguments[x] > 40
        ) {
          invalidPinError(arguments[x]);
    ...
    if (inputPin.length > 1) {
      ...
      inputPin.length < 2 &&
      options.array === true &&
      (options.index === "PinAsIndex" || options.index === "pin")
    ) {
      config =
        "{ pin:[" + inputPin + "], array:true, index:pin } " + endTime(1);
    } else if (inputPin.length < 2 && options.array === true) {
      config =
        "{ pin:[" + inputPin + "], array:true, index:0~n } " + endTime(1);
    } else {
      config = "{ pin:" + inputPin + " } " + endTime(1);
    }
  ...
  setOutput() {
    ...
    function createObject(outputPin, options, GpioOutput) {
      for (var x = 0; x < outputPin.length; x++) {
        /* Dec. 27, 2017 */
        var index = x;
        if (options) {
    ...
    if (outputPin.length > 1) {
      ...
      options &&
      options.array == true &&
      (options.index === "pin" || options === "PinAsIndex")
    ) {
      config =
        "{ pin:[" + outputPin + "], array:true, index:pin } " + endTime(1);
    } else if (outputPin.length < 2 && options && options.array) {
      config =
        "{ pin:[" + outputPin + "], array:true, index:0~n } " + endTime(1);
    } else {
      config = "{ pin:" + outputPin + " } " + endTime(1);
    }
      ...
      setImmediate(function () {
        ...
        if (pwmPin.c1.length > 1 && pwmObject > 1) {
          if (pwmPin.c1[1] === validPin) {
            console.log(
              "Paired PWM peripherals (pin " + pwmPin.c1 + ") detected."
            );
        ...
        if (pwmPin.c2.length > 1 && pwmObject > 1) {
          if (pwmPin.c2[1] === validPin) {
            console.log(
              "Paired PWM peripherals (pin " + pwmPin.c2 + ") detected."
            );
            console.log("Range and data will be same for both peripherals.\n");
          }
        }
